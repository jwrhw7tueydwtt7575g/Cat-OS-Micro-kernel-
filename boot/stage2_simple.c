// MiniSecureOS Stage 2 Bootloader - Simplified Version
...
static void enable_a20_line(void) {
    // Disable keyboard
    __asm__ volatile("inb %0, %%al" : : "Nd"(PORT_KEYBOARD_STATUS));
    __asm__ volatile("outb %%al, %0" : : "Nd"(PORT_KEYBOARD_STATUS));
    
    // Read output port
    __asm__ volatile("inb %0, %%al" : : "Nd"(PORT_KEYBOARD_DATA));
    
    // Write command to enable A20
    __asm__ volatile("movb $0xD1, %%al; outb %%al, %0" : : "Nd"(PORT_KEYBOARD_STATUS));
    
    // Enable A20 bit
    __asm__ volatile("movb $0xDF, %%al; outb %%al, %0" : : "Nd"(PORT_KEYBOARD_DATA));
    
    // Wait for completion
    for (uint32_t timeout = 100000; timeout > 0; timeout--) {
        uint8_t status;
        __asm__ volatile("inb %0, %%al" : "=a"(status) : "Nd"(PORT_KEYBOARD_STATUS));
...
static void read_disk(uint32_t lba, uint8_t sectors_param, void* buffer) {
    ...
    __asm__ volatile(
        ...
        "movb $0x00, %%dl\n"      // Drive A
        "int $0x13\n"
        "setc %0\n"               // Set carry flag in status
        : "=r"(status)
        : "b"(buffer), "r"(sectors_param), "r"(cylinder), "r"(sector), "r"(head)
        : "eax", "ecx", "edx"
    );
    
    if (status & 1) {
        print_string("Disk error reading kernel\r\n");
        while (1) {
            __asm__ volatile("hlt");
        }
    }
